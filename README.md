...
- `static/media-manifest.json` records source filenames, SHA-256 hashes, dimensions, duration, frame counts and poster timestamps. Thirteen MP4s hold their final frame for two additional seconds (60 frames at 30 fps); the full film is copied byte-for-byte from `DiagGen_v4_compressed.mp4` without an added hold. The complete content still plays at 1× with its existing framing. Four simulation videos have bottom-left close-up insets; Plush combines its two complete views into one side-by-side video. Source and pre-hold hashes remain recorded separately from delivery hashes where processing was applied.
